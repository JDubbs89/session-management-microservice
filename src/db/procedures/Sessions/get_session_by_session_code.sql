CREATE OR REPLACE FUNCTION get_session_by_session_code(_session_code TEXT, _user_username TEXT, _session_passcode TEXT DEFAULT '')
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM user_sessions WHERE host_username = _username) THEN
        RAISE EXCEPTION 'User is not hosting a session';
    END IF;
    ...
    IF _session.allow_join = 'friends only' AND _user_username != ALL (SELECT json_array_elements_text(_session.session_whitelist)) THEN
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION 'Session is friends only and user is not friends with the host';
        END IF;
    END IF;
    ...
    IF _session.session_passcode != _session_passcode AND _user_username != ALL (SELECT json_array_elements_text(_session.session_whitelist)) THEN
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION 'Session passcode invalid, and user is not friends with the host';
        END IF;
    END IF;
...
CREATE OR REPLACE FUNCTION does_session_exist(_session_code TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    _exists BOOLEAN;
BEGIN
